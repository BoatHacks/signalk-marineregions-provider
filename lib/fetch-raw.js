...
const { matchLayers, allLayerKeys } = require('./layers')
...
async function run({ layerKeys } = {}) {
  ...
  const allMatches = matchLayers(featureTypes)
  const matches = allMatches.filter((m) => wantedKeys.includes(m.layerKey))

  const unknownKeys = wantedKeys.filter((k) => !allLayerKeys().includes(k))
  if (unknownKeys.length > 0) {
    throw new Error(`Unknown layer key(s): ${unknownKeys.join(', ')}. Valid keys: ${allLayerKeys().join(', ')}`)
  }

  console.log(`\nFetching ${matches.length} of ${allMatches.length} available layer(s): ${wantedKeys.join(', ')}`)
  console.log(`\nMatch results:`)
  ...
  // Clean up any previously-fetched layers that are no longer selected, so
  // deselecting something in the admin UI actually reclaims disk space
  // instead of leaving orphaned files behind.
  for (const key of allLayerKeys()) {
    if (!wantedKeys.includes(key)) {
      const stalePath = path.join(SOURCES_DIR, `${key}.geojson`)
      if (fs.existsSync(stalePath)) {
        fs.unlinkSync(stalePath)
        console.log(`Removed ${stalePath} (layer no longer selected)`)
      }
    }
  }
